...
void main() {
  ...
  testWidgets(
      'If enabled is false, the mouse cursor should be deferred when hovered',
      (WidgetTester tester) async {
    ...
    await tester
        .pumpWidget(buildDropdownMenu(enabled: false, requestFocusOnTap: true));
    await tester.pumpAndSettle();
    ...
    await tester.pumpWidget(
        buildDropdownMenu(enabled: false, requestFocusOnTap: false));
    await tester.pumpAndSettle();
  ...
  testWidgets('DropdownMenu can have customized search algorithm',
      (WidgetTester tester) async {
    ...
    void checkExpectedHighlight(
        {String? searchResult, required List<String> otherItems}) {
      if (searchResult != null) {
        ...
        expect(
            itemMaterial.color, theme.colorScheme.onSurface.withOpacity(0.12));
      }
    ...
    await tester.pump();
    checkExpectedHighlight(searchResult: 'Unread', otherItems: <String>[
      'All',
      'Read'
    ...
    await tester.enterText(find.byType(TextField), 'read');
    await tester.pump();
    checkExpectedHighlight(searchResult: 'All', otherItems: <String>[
      'Unread',
      'Read'
    ...
    await tester.pumpWidget(dropdownMenu(
      searchCallback: (List<DropdownMenuEntry<int>> entries, String query) {
        ...
        final int index = entries
            .indexWhere((DropdownMenuEntry<int> entry) => entry.label == query);
    ...
    await tester.enterText(find.byType(TextField), 'read');
    await tester.pump();
    checkExpectedHighlight(otherItems: <String>[
      'All',
      'Unread',
      'Read'
    ...
    checkExpectedHighlight(
        searchResult: 'Read', otherItems: <String>['All', 'Unread']);
  });

  testWidgets(
      'onSelected gets called when a selection is made in a nested menu',
      (WidgetTester tester) async {
    int selectionCount = 0;
  ...
  testWidgets(
    ...
    await tester.fling(
        find.byType(ListView), const Offset(0, -20000), 200000.0);
  ...
  testWidgets('Menu shows scrollbar when height is limited',
      (WidgetTester tester) async {
    final List<DropdownMenuEntry<TestMenu>> menuItems =
        <DropdownMenuEntry<TestMenu>>[
      DropdownMenuEntry<TestMenu>(
  ...
  testWidgets('DropdownMenu.focusNode can focus text input field',
      (WidgetTester tester) async {
  ...
  testWidgets('DropdownMenu honors inputFormatters',
      (WidgetTester tester) async {
    int called = 0;
